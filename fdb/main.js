...
function renderCards() {
  ...
  filteredData.forEach(item => {
    ...
    card.innerHTML = `
      <div class="prog-img" style="background-image: url('${eventImg}'); background-position: ${bgPos};">
        <div class="prog-tag">
...
searchInput.addEventListener('input', (e) => {
  ...
  // Search in sections (title or keywords)
  sections.forEach(s => {
  ...
  // Search in programmation (name, description or category)
  programmationData.forEach(p => {
    const matchesName = p.nom.toLowerCase().includes(query);
    const matchesDesc = p.description.toLowerCase().includes(query);
    const matchesCat = p.categorie.toLowerCase().includes(query);
...
function renderSearchResults(results) {
  if (results.length === 0) {
    searchResults.innerHTML = '<div style="color: white; opacity: 0.6;">Aucun résultat trouvé...</div>';
    return;
  }
  ...
  searchResults.innerHTML = results.map(r => `
    <div class="search-result-item" data-id="${r.id}">
      ...
      ${r.snippet ? `<div class="result-snippet">${r.snippet.substring(0, 100)}...</div>` : ''}
    </div>
  `).join('');
